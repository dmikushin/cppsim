...
#pragma omp parallel for schedule(static)
    ...
    void run(){
        ...
        auto ptr = reinterpret_cast<complex_type*>(vec_);
        
        switch (ids.size()){
            case 1:
                #pragma omp parallel
                kernel(ptr, ids[0], m, ctrlmask);
                break;
            case 2:
                #pragma omp parallel
                kernel(ptr, ids[1], ids[0], m, ctrlmask);
                break;
            case 3:
                #pragma omp parallel
                kernel(ptr, ids[2], ids[1], ids[0], m, ctrlmask);
                break;
            case 4:
                #pragma omp parallel
                kernel(ptr, ids[3], ids[2], ids[1], ids[0], m, ctrlmask);
                break;
            case 5:
                #pragma omp parallel
                kernel(ptr, ids[4], ids[3], ids[2], ids[1], ids[0], m, ctrlmask);
                break;
            default:
                // Use embedded generator to generate larger gates in runtime
                kernelgen(ptr, ids, m, ctrlmask);
		break;
        }
